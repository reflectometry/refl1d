...
thetaM, thetaM2 = 90.0, 90.0  # non spin flip
sample = (
    silicon(0, s_mid)
    | nickel(
        100,
        s_mid,
        magnetism=Magnetism(rhoM=5, thetaM=thetaM, interface_below=interface_below, interface_above=interface_above),
    )
    | copper(10, 5)
...
rev_sample = (
    ...
    | nickel(sample[3].thickness, sample[2].interface, magnetism=Magnetism(rhoM=5, thetaM=thetaM2))
    | copper(sample[2].thickness, sample[1].interface)
    | nickel(
        sample[1].thickness,
        sample[0].interface,
        magnetism=Magnetism(rhoM=5, thetaM=thetaM, interface_below=interface_above, interface_above=interface_below),
    )
    | silicon
)
# del sample[2:5]
...
xs = [
    NeutronProbe(T=T * (-1 if name == "neg" else 1), dT=0.01, L=4.75, dL=0.0475, name=name)
...
M2 = Experiment(probe=fine, sample=sample, name="fine step", step_interfaces=True, dz=0.1)
M3 = Experiment(probe=coarse, sample=sample, name="coarse step", step_interfaces=True, dz=2)
M4 = Experiment(probe=neg, sample=rev_sample, name="neg", step_interfaces=True, dz=2)
