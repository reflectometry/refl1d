...
200	CONTINUE
	...
	DO 300 L=1,NL
	SUMT=SUMT+T(L)
	ARG1=CR*(4.*PI*(BN(L)+PN(L))-E0)
	ARG2=CR*(4.*PI*(BN(L)-PN(L))-E0)
	S(1)=CDSQRT(ARG1)
	S(3)=CDSQRT(ARG2)
...
480	CONTINUE
	...
	TM=(0.0,0.0)
	TP=(0.0,0.0)
	ARGZSP=CR*(4.*E0-16.*PI*ROSUP)
	ZSP=(CI/2.)*CDSQRT(ARGZSP)
	ARGZSM=CR*(4.*E0-16.*PI*ROSUM)
	ZSM=(CI/2.)*CDSQRT(ARGZSM)
	ARGZIP=CR*(4.*E0-16.*PI*ROINP)
	ZIP=(CI/2.)*CDSQRT(ARGZIP)
	ARGZIM=CR*(4.*E0-16.*PI*ROINM)
