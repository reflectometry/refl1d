...
C   u1 = ( B + Bx + iBy - Bz ) / ( B + Bx - iBy + Bz )
C   u3 = (-B + Bx + iBy - Bz ) / (-B + Bx - iBy + Bz )
C   Qc^2 = 16 pi rho
...
//    medium SLD.
      ...
      ZIP=CI*sqrt(CR*E0-PI4*(RHO[L]+RHOM[L]) + CI*PI4*IRHO[L]);
      ZIM=CI*sqrt(CR*E0-PI4*(RHO[L]-RHOM[L]) + CI*PI4*IRHO[L]);
//    Process the loop once for each interior layer, either from
//    front to back or back to front.
      for (I=1; I < N-1; I++) {
        L = L+STEP;
        S1 = sqrt(CR*PI4*(RHO[L]+RHOM[L])-E0 + CI*PI4*IRHO[L]);
        S3 = sqrt(CR*PI4*(RHO[L]-RHOM[L])-E0 + CI*PI4*IRHO[L]);
        U1L = U1[L];
        U3L = U3[L];
...
//    media --- use gepore.f directly for a more complete solution
      L=L+STEP;
//    Branch cut selection: this picks +ikz for positive real argument 
//    (above critical KZ), and -kz for negative real argument
//    (below the critical KZ)
      ZSP=CI*sqrt(CR*E0-PI4*(RHO[L]+RHOM[L]) + CI*PI4*IRHO[L]);
      ZSM=CI*sqrt(CR*E0-PI4*(RHO[L]-RHOM[L]) + CI*PI4*IRHO[L]);
...
//    Annotated with corresponding P matrix entries from Chatterji book chapt.

      V11=ZSP*A11+X*A31+YPP*A13-ZIP*A33; // = P3
      V12=ZSP*A12+X*A32+YMP*A14-ZIM*A34; // = P7
      V21=ZSM*A21+X*A41+YPM*A23-ZIP*A43; // = P1
      V22=ZSM*A22+X*A42+YMM*A24-ZIM*A44; // = P5

      W11=ZSP*A11+X*A31-YPP*A13+ZIP*A33; // = P2, P6, P12
      W12=ZSP*A12+X*A32-YMP*A14+ZIM*A34; // = P9
      W21=ZSM*A21+X*A41-YPM*A23+ZIP*A43; // = P4, P8, P10 
      W22=ZSM*A22+X*A42-YMM*A24+ZIM*A44; // = P11
