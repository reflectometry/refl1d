...
c
	...
	CR=(1.0,0.0)
	C0=(0.0,0.0)
	OPEN(UNIT=10,NAME='inpt.d',STATUS='OLD',FORM='FORMATTED')
	READ(10,*)NL,NC,QS,DQ,NQ,EPS,IP,IM,ROINP,ROINM,ROSUP,ROSUM
	WRITE(*,*)NL,NC,QS,DQ,NQ,EPS,IP,IM,ROINP,ROINM,ROSUP,ROSUM
	CLOSE(10)
	OPEN(UNIT=11,NAME='tro.d',STATUS='OLD',FORM='FORMATTED')
	READ(11,*)(T(J),BN(J),PN(J),THE(J),PHI(J),J=1,NL)
	CLOSE(11)
	IF(NQ.GT.1000)GO TO 900
	IF(NL.GT.1000)GO TO 900
	OPEN(UNIT=14,NAME='qrm2.d',STATUS='UNKNOWN',FORM='FORMATTED')
	OPEN(UNIT=15,NAME='qrp2.d',STATUS='UNKNOWN',FORM='FORMATTED')
	OPEN(UNIT=16,NAME='qtm2.d',STATUS='UNKNOWN',FORM='FORMATTED')
	OPEN(UNIT=17,NAME='qtp2.d',STATUS='UNKNOWN',FORM='FORMATTED')
	OPEN(UNIT=20,NAME='qrpmtpms.d',STATUS='UNKNOWN',FORM='FORMATTED')
	OPEN(UNIT=21,NAME='sum.d',STATUS='UNKNOWN',FORM='FORMATTED')
	OPEN(UNIT=31,NAME='rrem.d',STATUS='UNKNOWN',FORM='FORMATTED')
	OPEN(UNIT=32,NAME='rimm.d',STATUS='UNKNOWN',FORM='FORMATTED')
	OPEN(UNIT=33,NAME='rrep.d',STATUS='UNKNOWN',FORM='FORMATTED')
	OPEN(UNIT=34,NAME='rimp.d',STATUS='UNKNOWN',FORM='FORMATTED')
	OPEN(UNIT=41,NAME='rpolx.d',STATUS='UNKNOWN',FORM='FORMATTED')
	OPEN(UNIT=42,NAME='tpolx.d',STATUS='UNKNOWN',FORM='FORMATTED')
	OPEN(UNIT=43,NAME='rpoly.d',STATUS='UNKNOWN',FORM='FORMATTED')
	OPEN(UNIT=44,NAME='tpoly.d',STATUS='UNKNOWN',FORM='FORMATTED')
	OPEN(UNIT=45,NAME='rpolz.d',STATUS='UNKNOWN',FORM='FORMATTED')
	OPEN(UNIT=46,NAME='tpolz.d',STATUS='UNKNOWN',FORM='FORMATTED')
...
200	CONTINUE
	...
	DO 300 L=1,NL
	SUMT=SUMT+T(L)
	ARG1=CR*(4.*PI*(BN(L)+PN(L))-QP*QP/4.)
	ARG2=CR*(4.*PI*(BN(L)-PN(L))-QM*QM/4.)
	S(1)=CDSQRT(ARG1)
	S(3)=CDSQRT(ARG2)
...
480	CONTINUE
	...
	TM=(0.0,0.0)
	TP=(0.0,0.0)
	ARGZSP=CR*(QP*QP-16.*PI*ROSUP)
	ZSP=(CI/2.)*CDSQRT(ARGZSP)
	ARGZSM=CR*(QM*QM-16.*PI*ROSUM)
	ZSM=(CI/2.)*CDSQRT(ARGZSM)
	ARGZIP=CR*(QP*QP-16.*PI*ROINP)
	ZIP=(CI/2.)*CDSQRT(ARGZIP)
	ARGZIM=CR*(QM*QM-16.*PI*ROINM)
	ZIM=(CI/2.)*CDSQRT(ARGZIM)
	X=-1.*CR
	YPP=ZIP*ZSP
	YMM=ZIM*ZSM
	YPM=ZIP*ZSM
