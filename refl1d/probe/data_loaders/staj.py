...
class MlayerModel(object):
    ...
    **Constructing new files:**
    ...
    def FWHMresolution(self, Q):
        ...
        """
        return (abs(Q) * self.wavelength_dispersion + 4 * pi * self.angular_divergence) / self.wavelength
    ...
    def fit_FWHMresolution(self, Q, dQ, weight=1):
        ...
        """
        A = np.array([abs(Q) / self.wavelength, np.ones_like(Q) * (4 * pi / self.wavelength)]).T
    ...
    def __str__(self):
        ...
        if self.data_file != "":
            ...
            line.append("Q: %g to %g in %d steps" % (self.Qmin, self.Qmax, self.num_Q))
        line.append(
            "Wavelength L: %g  dL/L: %g, dTheta: %g"
            % (self.wavelength, self.wavelength_dispersion, self.angular_divergence)
        )
        line.append(
            "Beam intensity: %g  Background: %g, Theta offset: %g"
            % (self.intensity, self.background, self.theta_offset)
        )
        profile = "error function" if self.roughness_profile == "E" else "tanh"
        line.append("Interface: %s in %d steps" % (profile, self.roughness_steps))
        w, s = self.thickness, self.roughness
        rho, mu = self.rho, self.mu
        line.append("Layers:")
        line.append(("   " + "%15s " * 4) % ("Width (A)", "Interface (FWHM)", "Rho (1e-6/A)", "Mu (1e-6/A)"))
    ...
    def _check(self):
        ...
        """
        if (
            (self.irho is not None and len(self.rho) != len(self.irho))
    ...
    def _parse(self, lines):
        ...
        self.num_top, self.num_middle, self.num_bottom, self.num_repeats, _, self.roughness_steps = nums

        # 2: wavelength  wavelength_dispersion  angular_divergence [theta_offset]
        nums = [float(s) for s in lines[1].split()]
        self.wavelength, self.wavelength_dispersion, self.angular_divergence = nums[:3]
        ...
        layers = [[float(v) for v in line.split()] for line in lines[6 : 6 + nL]]
    ...
    def _write(self, fid):
        ...
        fid.write(
            ...
            % (
                ...
                self.num_repeats,
                len(self.fitpars),
                self.roughness_steps,
            )
        )

        # 2: wavelength  wavelength_dispersion  angular_divergence [theta_offset]
        fid.write(
            "%g %g %g %g\n" % (self.wavelength, self.wavelength_dispersion, self.angular_divergence, self.theta_offset)
        )

        # 3: intensity  background  Qmin  Qmax  num_Q
        fid.write("%g %g %g %g %d\n" % (self.intensity, self.background, self.Qmin, self.Qmax, self.num_Q))
        ...
        def _write_layer(idx):
            fid.write("%g %g %g %g %g\n" % (rho[idx], 0.0, w[idx], s[idx], mu[idx]))
...
class MlayerMagnetic(object):
    ...
    **Constructing new files:**
    ...
    def fit_FWHMresolution(self, Q, dQ, weight=1):
        A = np.array([abs(Q) / self.wavelength, np.ones_like(Q) * (4 * pi / self.wavelength)])
        s = wsolve(A, y=dQ, dy=weight)
        self.wavelength_dispersion = s.x[0]
        self.angular_divergence = s.x[1]
    ...
    def __str__(self):
        line = []
        if self.data_file != "":
            line.append("Data: %s[%s]" % (self.data_file, self.active_xsec.upper()))
        else:
            line.append("Q: %g to %g in %d steps" % (self.Qmin, self.Qmax, self.num_Q))
        line.append(
            "Wavelength L: %g  dL/L: %g, dTheta: %g"
        ...
        line.append(
            "Beam intensity: %g  Background: %g, Guide angle: %g" % (self.intensity, self.background, self.guide_angle)
        )
        profile = "error function" if self.roughness_profile == "E" else "tanh"
        line.append("Interface: %s in %d steps" % (profile, self.roughness_steps))
        w, s = self.thickness, self.roughness
        ...
        for i in range(len(self.rho)):
            line.append(
                ("%3d:" + ("%15g " * 4) + "\n    " + ("%15g " * 4))
                % (i, w[i], s[i], rho[i], irho[i], wm[i], sm[i], mrho[i], mtheta[i])
            )
        if self.constraints != "":
            line.append("Constraints:")
            line.append(self.constraints)
    ...
    def _parse(self, lines):
        ...
        # Line next (3):  mtheta
        nL = maxLayer + 1
        layers = [[float(v) for v in " ".join(lines[11 + 3 * i : 11 + 3 * (i + 1)]).split()] for i in range(nL)]
    ...
    def _write(self, fid):
        # 1: wavelength  wavelength_dispersion  angular_divergence [aguide]
        fid.write(
            "%g %g %g %g\n" % (self.wavelength, self.wavelength_dispersion, self.angular_divergence, self.guide_angle)
        )
        ...
        # 3: maxLayer  nRoughSteps  nFitParam
        fid.write("%d %d %d\n" % (len(self.rho) - 1, self.roughness_steps, len(self.fitpars)))
